...
func Execute_game(version string, assetID string, version_name string, username string, java_path string, work_path string, system_info string, system_arch string, Launcher_Version string, version_isolation bool, Xmn string, Xmx string) string {
	...
	index_name := gjson.Get(version_json, "assetIndex.id").String()
	...
	java_cmd += "'--assetIndex' '" + index_name + "' "
